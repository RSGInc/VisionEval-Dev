...
fetch_acsdata = function(autocfg, bg_list) {
  ...
  acs_variables <- fread(file.path(VEModel::getRuntimeDirectory(),
                                   'populationsim',
...
fetch_pumsdata = function(autocfg) {
  ...
  pums_vars <- fread(file.path(VEModel::getRuntimeDirectory(),
                               'populationsim',
...
setup_populationsim = function(
  ...
  TEMPLATE_DIR = file.path(RUNTIME_DIR, cfg$POPSIM_TEMPLATE_DIR)
  
  # Check source data validity
  CENSUS_SOURCE_DATA = lapply(names(cfg$AUTO_DATA_PREP$SOURCE_DATA$CENSUS), function(file_name) {
    FP = file.path('populationsim',cfg$AUTO_DATA_PREP$SOURCE_DATA$CENSUS[[file_name]][["FILE"]])
  ...
  file.copy(from=TEMPLATE_DIR, to=dirname(MODEL_DIR), recursive = TRUE)
  success <- file.rename(file.path(dirname(MODEL_DIR),'populationsim_template'), POPSIM_DIR)
  ...
  # HH/PER vars
  pums_vars <- fread(file.path(RUNTIME_DIR,
                               "populationsim",
...
setup_vemodel <- function(cfg_dir='../runtime/populationsim/popsim_settings.yaml'){
  ...
  hh_dt <- fread(file.path(POPSIM_DIR, cfg$POPSIM_OUTPUTS$HOUSEHOLD$FILE))
  per_dt <- fread(file.path(POPSIM_DIR, cfg$POPSIM_OUTPUTS$PERSON$FILE))
  ...
  # model_spec <- processModuleSpecs(model_spec)
  # processInputFiles(model_spec)
  ...
  RunStatus <- try (
    {
      ...
      visioneval::loadModel(popsimmodel$modelStages[[POPSIM_MODEL_NAME]][["RunParam_ls"]])
      ...
      # inputsToDatastore(Inp_ls, model_spec, "VEPopulationSim")
      # L <- getFromDatastore(processModuleSpecs(model_spec), popsimmodel$RunParam_ls$Years[1])
      # setInDatastore(Inp_ls)
      # L <- initDataList()
      # L$G <- getModelState()
      # L$G$Year <- popsimmodel$RunParam_ls$Years[1]
      # L$Global <- Inp_ls$Data$Global
      # Out_ls <- ReadPopulationSimOutput(L)
      # setInDatastore(Out_ls, model_spec, "VEPopulationSim", Year = L$G$Year)
  ...
  viewSetup(Param_ls = popsimmodel2_config)
