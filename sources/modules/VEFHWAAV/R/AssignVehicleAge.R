...
#Create a list with distributions of automated vehicles by age
#-------------------------------------------------------------
AvAgeModel_ls <- list()

#AV age distribution data
#------------------------
...
AvAge_df <-
  processEstimationInputs(
    ...
    "AssignVehicleAge.R")
AvAgeModel_ls <- list(
  Auto = setNames(AvAge_df$Auto, AvAge_df$Age),
  LtTrk = setNames(AvAge_df$LtTrk, AvAge_df$Age)
)
rm(Inp_ls, AvAge_df)

#Save the AV age model
#---------------------
#' AV age model
#'
#' A list containing the AV age model probability tables
#' needed to implement the AV age model.
#'
#' @format A list having the following components:
#' \describe{
#'   \item{Auto}{a vector of cumulative probability of AV autos by age}
#'   \item{LtTrk}{a vector of cumulative probability of AV light trucks by age}
#' }
#' @source AssignVehicleAge.R script.
"AvAgeModel_ls"
visioneval::savePackageDataset(AvAgeModel_ls, overwrite = TRUE)
...
AssignVehicleAge <- function(L) {
  ...
  #Iterate by Azone and assign vehicle age for non-automated vehicles
  #------------------------------------------------------------------
  ...
  for (az in Az) {
    #Create data frame for owned and non-automated (L0) vehicles
    UseOwn <- with(L$Year$Vehicle, Azone == az & VehicleAccess == "Own" & AVLvl == "L0")
