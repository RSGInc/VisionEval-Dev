...
CalculateRoadDvmtSpecifications <- list(
  ...
  Set = items(
    item(
      ...
      PROHIBIT = c("< 0", "> 1"),
      ISELEMENTOF = "",
      DESCRIPTION = "Proportion of DVMT of light duty vehicles with AV level 5 capabilities for households in each Marea."
    ),
    item(
      NAME = "LdvAVLvl3Prop",
      TABLE = "Marea",
      ...
      PROHIBIT = c("< 0", "> 1"),
      ISELEMENTOF = "",
      DESCRIPTION = "Proportion of DVMT of light duty vehicles with AV level 3 capabilities for households in each Marea."
    ),
    item(
      ...
      PROHIBIT = c("< 0", "> 1"),
      ISELEMENTOF = "",
      DESCRIPTION = "Proportion of household DVMT that is vehicles with AV level 5 capabilities in each Marea."
    ),
    item(
      ...
      PROHIBIT = c("< 0", "> 1"),
      ISELEMENTOF = "",
      DESCRIPTION = "Proportion of household DVMT that is vehicles with AV level 3 capabilities in each Marea."
    ),
    item(
      NAME = "HvyTrkDvmt",
      TABLE = "Region",
...
CalculateRoadDvmt <- function(L) {
  ...
  for (ma in Ma) {
    HhAVLvl3DvmtProp_Ma[ma] <- local({
      IsMa <- L$Year$Household$Marea == ma
      Dvmt_Hh <- L$Year$Household$Dvmt[IsMa]
      AVLvl3Prop_Hh <- L$Year$Household$AVLvl3DvmtProp[IsMa]
      sum(AVLvl3Prop_Hh * Dvmt_Hh) / sum(Dvmt_Hh)
    })
  }
  ...
  LdvAVLvl5Prop_Ma <-  HhAVLvl5DvmtProp_Ma * HhDvmtWts_Ma 
  LdvAVLvl3Prop_Ma <-  HhAVLvl3DvmtProp_Ma * HhDvmtWts_Ma #+ 
