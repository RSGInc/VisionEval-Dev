...
AssignVehicleOwnershipSpecifications <- list(
  ...
  Inp = items(
    ...
    item(
      ...
      TOTAL = "",
      DESCRIPTION =
        "Proportion of households with two vehicles",
      OPTIONAL = TRUE
    ),
    item(
      NAME = "Prop3Veh",
      ...
      TOTAL = "",
      DESCRIPTION =
        "Proportion of households with three vehicles",
      OPTIONAL = TRUE
    ),
    item(
      NAME = "Prop4Veh",
      ...
      TOTAL = "",
      DESCRIPTION =
        "Proportion of households with four vehicles",
      OPTIONAL = TRUE
    ),
    item(
      NAME = "Prop5Veh",
      ...
      TOTAL = "",
      DESCRIPTION =
        "Proportion of households with five vehicles",
      OPTIONAL = TRUE
    ),
    item(
      NAME = "Prop6Veh",
      ...
      TOTAL = "",
      DESCRIPTION =
        "Proportion of households with six or more vehicles",
      OPTIONAL = TRUE
    )
  ),
...
AssignVehicleOwnership <- function(L) {
  ...
  #---------------------------------
  AutoOwnModels_ls <- VEHouseholdVehiclesCCRPC::AutoOwnModels_ls
  ...
  #NEW: Vehicle adjustment code using vehicle ownership proportions
  #----------------------------------------------------------------
  if (!(is.na(L$Year$Region$Prop0Veh))) {
    TargetProp <- c(L$Year$Region$Prop0Veh,
                    L$Year$Region$Prop1Veh,
                    L$Year$Region$Prop2Veh,
                    L$Year$Region$Prop3Veh,
                    L$Year$Region$Prop4Veh,
                    L$Year$Region$Prop5Veh,
                    L$Year$Region$Prop6Veh)
    ChgVehCat <- 0:6
    NumHhByCategory_ <- sapply(ChgVehCat, function(x) sum(Vehicles_ == x))
    PropHhByCategory_ <- NumHhByCategory_ / sum(NumHhByCategory_)
    ChgVehByCategory_ <- TargetProp - PropHhByCategory_
    ChgVehByCategory_ <- round(ChgVehByCategory_ * sum(NumHhByCategory_))
    #Initialize a vector of changes
    for (Cat in ChgVehCat) {
      VehiclesChg_ <- integer(length(Vehicles_))
      NumToChg <- ChgVehByCategory_[Cat + 1]
      if (Cat == 0) {
        NumToChg <- NumToChg * -1
        HhIdxToChg_ <- which(Vehicles_ == Cat)
        ChgProb_ <- VehicleProb_HhNv[HhIdxToChg_, Cat + 1]
      }
      if (Cat > 0 && Cat < 6) {
        HhIdxToChg_ <- which(Vehicles_ == Cat - 1 | Vehicles_ == Cat + 1)
        ChgProb_ <- VehicleProb_HhNv[HhIdxToChg_, Cat]
      }
      if (Cat == 6) {
        HhIdxToChg_ <- which(Vehicles_ == Cat)
  ...
  #Fix error values
  Vehicles_[Vehicles_ == 7] <- 6
  Vehicles_[Vehicles_ == -1] <- 0
