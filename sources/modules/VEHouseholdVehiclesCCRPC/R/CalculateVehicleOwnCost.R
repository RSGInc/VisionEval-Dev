...
calcVehDepr <- function(Type_, Age_, Vmt_) {
  ...
  if ( ! exists("VehOwnCost_ls") ) VehOwnCost_ls <- VEHouseholdVehiclesCCRPC::VehOwnCost_ls
...
calcVehFin <- function(Type_, Age_) {
  ...
  VehOwnCost_ls <- VEHouseholdVehiclesCCRPC::VehOwnCost_ls
...
calcAdValoremTax <- function(Type_, Age_, TaxRate) {
  ...
  #Apply the index to calculate vehicle finance cost
  with(VEHouseholdVehiclesCCRPC::VehOwnCost_ls, Value_AgVt[ValueIdx_mx]) * TaxRate
}
...
idPaydHh <- function(Household, PaydHhProp) {
  ...
  #Return all qualifying households if less than or equal to NumPayd
    ...
    PaydWts_ <- VEHouseholdVehiclesCCRPC::PaydWts_
