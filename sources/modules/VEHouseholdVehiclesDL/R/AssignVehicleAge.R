...
AssignVehicleAge <- function(L) {
  ...
  for (az in Az) {
    ...
    if(nrow(Own_df) > 0){
      ...
      NumVeh_IgTy <- with(Own_df, table(factor(IncGrp, levels = Ig), 
                                        Type=factor(Type, levels = c("Auto", "LtTrk"))))
      ...
      # Get number of Autos and LtTrk
      # NumTypes_ <- table(factor(Own_df$Type, levels = c("Auto","LtTrk")))
      for (ig in Ig) {
        if(NumVeh_IgTy[ig, "Auto"]>0){
          Ages_ <-
            sample(
              0:30,
              NumVeh_IgTy[ig, "Auto"],
  ...
  if(nrow(Own_df) > 0){
    if(MaxAVVehYear == 0){
      ...
      Own_df$Age <- NA
      AVNumVeh_IgTy <- with(Own_df, table(factor(IncGrp, levels = Ig), 
                                          Type=factor(Type, levels = c("Auto", "LtTrk"))))
