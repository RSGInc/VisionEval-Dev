#=====================
#Make2017NHTSDataset.R
#=====================
#This module creates a data frame of data from the publically available data
#from the 2017 National Household Travel Survey (NHTS). The package produces a
#data frame of values by household.

#The data re not augmented with data on metropolitan area freeway supply and 
#transit supply.
...
#This script processes 2017 NHTS text files to create the household travel
#dataset to be used in model estimation. A household dataframe (Hh_df)
...
if ( dir.exists("data-raw") ) {
  ...
  #----------------------------
  #NHTS 2017 public use datasets were downloaded and unzipped in 
  Nhts2017Dir <- "inst/extdata"
  ...
  if (!file.exists("data-raw/Hh_df.rda")) {
    Hh_df <- read.csv(file.path(Nhts2017Dir, "hhpub.csv"))
    ### TODO For now keep all of the variables
    ###      This is still the list for the 2001 data, 2017 is different
  ...
  AllTripsHh_ <- Hh_df$HOUSEID[!is.na(Hh_df$WTHHFIN)]
  ...
  if (!file.exists("data-raw/Veh_df.rda")) {
    Veh_df <- read.csv(file.path(Nhts2017Dir, "vehpub.csv"))
    ### TODO For now keep all of the variables
    ###      This is still the list for the 2001 data, 2017 is different
    # Keep_ <-
  ...
  if (!file.exists("data-raw/Per_df.rda")) {
    Per_df <- read.csv(file.path(Nhts2017Dir, "perpub.csv"))
    ### TODO For now keep all of the variables
    ###      This is still the list for the 2001 data, 2017 is different
    # Keep_ <-
  ...
  if (!file.exists("data-raw/Dt_df.rda")) {
    Dt_df <- read.csv(file.path(Nhts2017Dir, "trippub.csv"))
    ### TODO For now keep all of the variables
    ###      This is still the list for the 2001 data, 2017 is different
    # Keep_ <-
  ...
  #------------------------------------------------------------------------------
  # NHTS 2017: age is in the persons table
  #Per_df[,c("Houseid", "Personid", "R_age", "R_age_imp")]
  ...
  AgeBreaks <- c(0, 14, 19, 29, 54, 64, max(Per_df$R_age_imp + 1, na.rm = TRUE))
  # Allocated persons to categories
  Per_df$AgeGroup <- findInterval(Per_df$R_age_imp, vec = AgeBreaks)
  ...
  Ages_HhAg <- dcast.data.table(data = data.table(Per_df[,c("Houseid", "AgeGroup")]),
                                formula = Houseid ~ AgeGroup,
                                fun.aggregate = length,
                                value.var = "AgeGroup")
  setnames(Ages_HhAg, c("Houseid", "Age0to14", "Age15to19", 
                        "Age20to29", "Age30to54", "Age55to64", "Age65Plus"))
  
  # Add age category data to Hh_df
  ...
  #------------------------------------------------------------------------------
  # NHTS 2017: driver status is in the persons table
  #Per_df[,c("Houseid", "Personid", "Driver")]
  # 1=Yes, 2=No, NA=skipped (e.g., too young)
  Per_df$R_Driver <- ifelse(Per_df$Driver == 1, 1, NA)
  ...
  Drvs_HhAg <- dcast.data.table(data = data.table(Per_df[,c("Houseid", "R_Driver", "AgeGroup")])[AgeGroup > 1],
                               formula = Houseid ~ AgeGroup,
                               fun.aggregate = sum,
                               value.var = "R_Driver",
                               fill = NA)
  Drvs_HhAg[is.na(Drvs_HhAg)] <- 0
  
  setnames(Drvs_HhAg, c("Houseid", "Drv15to19", "Drv20to29", 
                        "Drv30to54", "Drv55to64", "Drv65Plus"))
  
  # Add drive age data to Hh_df
  ...
  #------------------------------------------------------------------------------
  # NHTS 2017: worker status is in the persons table
  #Per_df[,c("Houseid", "Personid", "Worker")]
  # 1=Yes, 2=No, NA=skipped
  Per_df$R_Worker <- ifelse(Per_df$Worker == 1, 1, NA)
  ...
  Wkrs_HhAg <- dcast.data.table(data = data.table(Per_df[,c("Houseid", "R_Worker", "AgeGroup")])[AgeGroup > 1],
                               formula = Houseid ~ AgeGroup,
                               fun.aggregate = sum,
                               value.var = "R_Worker",
                               fill = NA)
  Wkrs_HhAg[is.na(Wkrs_HhAg)] <- 0
  
  setnames(Wkrs_HhAg, c("Houseid", "Wkr15to19", "Wkr20to29", 
                       "Wkr30to54", "Wkr55to64", "Wkr65Plus"))
  
  # Add working status age data to Hh_df
  ...
  rm(AgeBreaks, Ages_HhAg, Drvs_HhAg, Wkrs_HhAg)

  #Process household income data
  #-----------------------------
  # Categories updates for 2017 NHTS
  #sort(unique(Hh_df$Hhfaminc))
  
  #Define income groupings from NHTS code book
  #Assume top value of high income group to be 250K
  IncGrp_ls <-
    list(
      c(0, 9999),
      c(10000, 14999),
  ...
  Hh_df$Income <- MidPtInc_[Hh_df$Hhfaminc]
  rm(IncGrp_ls, MidPtInc_)
  #Assign income group variable
  IncBreaks_ <- c( 0, 20000, 40000, 60000, 80000, 100000, 250000 )
  ...
  NumAuto_Hh <- data.frame(Houseid = names(NumAuto_Hh), 
                              NumAuto = NumAuto_Hh)
  NumLightTruck_Hh <- data.frame(Houseid = names(NumLightTruck_Hh), 
                                 NumLightTruck = NumLightTruck_Hh)
  ...
  Veh_df$Gscostmile2 <- with(Veh_df, Gscost / Fegempg)
  ...
  getPersonTours <- function(PTrp_df, replaceNA = FALSE) {
    ...
    do.call(rbind, lapply(Tours_ls, function(x) {
      data.frame(
        ...
        TravelTime = sum(x$Trvlcmin, na.rm = TRUE),
        #DwellTime = sum(x$Trwaittm, na.rm = TRUE),
        ...
        Disttowk = Per_df$Disttowk17[Per_df$Personid == x$Personid[1]],
        Signature =
  ...
  FieldsToCheck_ <-
    c("Houseid", "Distance", "TravelTime", "StartHome", "EndHome",
  ...
  IsInLimits_ <-
    with(HhTours_df,
         Distance != 0 & Distance <= quantile(Distance, 0.99) &
           ...
           #DwellTime != 0 & DwellTime <= quantile(DwellTime, 0.99) ### Don't think dwelltime is correctly coded, don't use
    )
  ...
  #Add a Mode variable
  HhTours_df <- HhTours_df[HhTours_df$Trptrans > 0 & HhTours_df$Trptrans != 97,]
  HhTours_df <- HhTours_df[,]
  #NHTS 2017 mode coding
  Modes_ <- c("1" = "Walk", "2" = "Bicycle", "3" = "Auto", "4" = "LtTrk",
              "5" = "LtTrk", "6" = "LtTrk", "7" = "GolfCartSegway", "8" = "Motorcycle",
              "9" = "RV", "10" = "SchoolBus", "11" = "Bus", "12" = "Bus",
              "13" = "Bus", "14" = "Bus", "15" = "Train", "16" = "Subway",
              "17" = "Taxi", "18" = "Taxi", "19" = "Airplane", "20" = "Boat")
  # #old mode list
  ...
  IncludesWork_ <-
    sapply(HhTours_df$Whyto, function(x) {
      any(unlist(strsplit(x, "-")) %in% c("2", "3", "4"))
    })
  HhTours_df$IncludesWork <- unname(IncludesWork_)
  rm(IncludesWork_)
  
  # Create a trip level summary as opposed to a tour level summary
  # This accounts for mixed tours with multiple modes
  # Can use to extract the correct set of transit, bike, and walk trips
  HhTrips_df <- data.table(Dt_df[,c("Houseid", "Personid", "Trpmiles", "Trvlcmin", "Trptrans", "Trphhveh")])
  setnames(HhTrips_df, c("Trpmiles", "Trvlcmin", "Trphhveh"), c("Distance", "Traveltime", "HhVehUsed"))

  HhTrips_df <- HhTrips_df[Distance != 0 & Traveltime != 0]
  HhTrips_df <- HhTrips_df[Trptrans > 0 & Trptrans != 97]
  ...
  HhTrips_df[, Mode := Modes_[Trptrans]]
  ...
  PvtVehDvmt_Hh <- data.frame(Houseid = names(PvtVehDvmt_Hh), 
                              PvtVehDvmt = PvtVehDvmt_Hh)
  PvtVehTrips_Hh <- data.frame(Houseid = names(PvtVehTrips_Hh), 
                               PvtVehTrips = PvtVehTrips_Hh)
  ...
  ShrVehDvmt_Hh <- data.frame(Houseid = names(ShrVehDvmt_Hh), 
                              ShrVehDvmt = ShrVehDvmt_Hh)
  ShrVehTrips_Hh <- data.frame(Houseid = names(ShrVehTrips_Hh), 
                               ShrVehTrips = ShrVehTrips_Hh)
  ...
  # Add distinction between private and shared travel in vehicles based on the trip data to account for mixed tours
  # Summary from the trip data
  HhTrips_df[, IsPvtVehTravel := ifelse(Mode %in% c("Auto", "LtTrk", "OthTrk", "RV", "Motorcycle") & HhVehUsed == 1,1,0)]
  PvtVehDpmtTrips_Hh <- HhTrips_df[IsPvtVehTravel == 1, .(PvtVehDpmt = sum(Distance),
                                                      PvtVehTripsTr = .N),
                                 keyby = Houseid]
  ...
  Hh_df$PvtVehDpmt[is.na(Hh_df$PvtVehDpmt)] <- 0
  Hh_df$PvtVehTripsTr[is.na(Hh_df$PvtVehTripsTr)] <- 0
  
  HhTrips_df[, IsShrVehTravel := ifelse(Mode %in% c("Taxi") | (Mode %in% c("Auto", "LtTrk", "OthTrk", "RV", "Motorcycle") & HhVehUsed == 2),1,0)]
  ShrVehDpmtTrips_Hh <- HhTrips_df[IsShrVehTravel == 1, .(ShrVehDpmt = sum(Distance),
                                                          ShrVehTripsTr = .N),
                                   keyby = Houseid]
  ...
  Hh_df$ShrVehDpmt[is.na(Hh_df$ShrVehDpmt)] <- 0
  Hh_df$ShrVehTripsTr[is.na(Hh_df$ShrVehTripsTr)] <- 0
  ...
  # WalkDpmt_Hh <- data.frame(Houseid = names(WalkDpmt_Hh), 
  #                           WalkDpmt = WalkDpmt_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                WalkDpmt_Hh,
  #                by = "Houseid",
  ...
  # 
  # WalkTrips_Hh <-
  #   unlist(tapply(HhTours_df$Trips[IsWalkTravel_], HhTours_df$Houseid[IsWalkTravel_], sum))
  # 
  # WalkTrips_Hh <- data.frame(Houseid = names(WalkTrips_Hh), 
  #                            WalkTrips = WalkTrips_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                WalkTrips_Hh,
  #                by = "Houseid",
  #                all.x = TRUE)
  # 
  # Hh_df$WalkTrips[is.na(Hh_df$WalkTrips)] <- 0
  # 
  # rm(IsWalkTravel_, WalkDpmt_Hh, WalkTrips_Hh)
  
  # Summary from the trip data
  HhTrips_df[, IsWalkTravel := ifelse(Mode == "Walk",1,0)]
  WalkDpmtTrips_Hh <- HhTrips_df[IsWalkTravel == 1, .(WalkDpmt = sum(Distance),
                                                      WalkTrips = .N),
                                 keyby = Houseid]
  ...
  # 
  # BicycleDpmt_Hh <- data.frame(Houseid = names(BicycleDpmt_Hh), 
  #                              BikeDpmt = BicycleDpmt_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                BicycleDpmt_Hh,
  #                by = "Houseid",
  ...
  # 
  # BikeTrips_Hh <-
  #   unlist(tapply(HhTours_df$Trips[IsBicycleTravel_], HhTours_df$Houseid[IsBicycleTravel_], sum))
  # 
  # BikeTrips_Hh <- data.frame(Houseid = names(BikeTrips_Hh), 
  #                            BikeTrips = BikeTrips_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                BikeTrips_Hh,
  #                by = "Houseid",
  #                all.x = TRUE)
  # 
  # Hh_df$BikeTrips[is.na(Hh_df$BikeTrips)] <- 0
  # rm(IsBicycleTravel_, BicycleDpmt_Hh, BikeTrips_Hh)
  
  # Summary from the trip data
  HhTrips_df[, IsBikeTravel := ifelse(Mode == "Bicycle",1,0)]
  BikeDpmtTrips_Hh <- HhTrips_df[IsBikeTravel == 1, .(BikeDpmt = sum(Distance),
                                                      BikeTrips = .N),
                                 keyby = Houseid]
  ...
  # 
  # TransitDpmt_Hh <- data.frame(Houseid = names(TransitDpmt_Hh), 
  #                              TransitDpmt = TransitDpmt_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                TransitDpmt_Hh,
  #                by = "Houseid",
  ...
  # 
  # TransitTrips_Hh <- data.frame(Houseid = names(TransitTrips_Hh), 
  #                               TransitTrips = TransitTrips_Hh)
  # 
  # Hh_df <- merge(Hh_df,
  #                TransitTrips_Hh,
  #                by = "Houseid",
  ...
  # Summary from the trip data
  HhTrips_df[, IsTransitTravel := ifelse(Mode %in% c("Bus", "SchoolBus", "Train", "Subway", "StreetCar"),1,0)]
  TransitDpmtTrips_Hh <- HhTrips_df[IsTransitTravel == 1, .(TransitDpmt = sum(Distance),
                                                      TransitTrips = .N),
                                 keyby = Houseid]
  ...
  # Second version based on the trips table field, not tours
  Hh_df$ZeroDpmt <- "N"
  Hh_df$ZeroDpmt[Hh_df$PvtVehDpmt == 0 & Hh_df$ShrVehpvmt == 0] <- "Y"
  Hh_df$ZeroDpmt <- as.factor(Hh_df$ZeroDpmt)
  ...
  ### TODO This is already done above, check to make sure code is consistent before removing this code
  #Assign age category
  AgeBreaks <- c(0, 14, 19, 29, 54, 64, max(Per_df$R_age_imp + 1, na.rm = TRUE))
  AgeGroups_ <-
  ...
  # NHTS 2017 does not have commercial driver question
  ...
  Nwalktrp_Hh <- data.frame(Houseid = names(Nwalktrp_Hh), 
                            Nwalktrp = Nwalktrp_Hh)
  ...
  Usepubtr_Hh <- data.frame(Houseid = names(Usepubtr_Hh), 
                            Usepubtr = Usepubtr_Hh)
  ...
  #Clean up
  rm(Per_ls, Nbiketrp_Hh, Nwalktrp_Hh, Usepubtr_Hh)
  ...
  #Hh_df$Hh_race <- factor( Hh_df$Hh_race )
  ...
  Hh_df$Lif_cyc[is.na(Hh_df$Lif_cyc)] <- "-9"
  ...
  Hh_df$Msacat <- factor(Hh_df$Msacat,
                         levels = 1:4,
                         labels = c("MSA of 1 million or more, with rail",
                                    "MSA of 1 million or more, and not in 1",
                                    "MSA less than 1 million",
                                    "Not in MSA"))
  
  Hh_df$Msasize <- factor(Hh_df$Msasize,
                          levels = 1:6,
                          labels = c("In an MSA of Less than 250,000",
                          "In an MSA of 250,000 - 499,999",
                          "In an MSA of 500,000 - 999,999",
                          "In an MSA or CMSA of 1,000,000 - 2,999,999",
                          "In an MSA or CMSA of 3 million or more",
                          "Not in MSA or CMSA"))
  
  Hh_df$Rail <- factor(Hh_df$Rail,
                       levels = 1:2,
                       labels = c("MSA has rail",
                                  "MSA does not have rail, or hh not in an MSA"))
...
### TODO update this to reflect NHTS 2017
...
### TEMP: save as RDA File
if ( exists("Hh_df") ) save(Hh_df, file = "data-raw/Hh_df.Rda")
...
### TEMP: save as RDA File
if ( exists("Veh_df") ) save(Veh_df, file = "data-raw/Veh_df.Rda")
...
### TEMP: save as RDA File
if ( exists("HhTours_df") ) save(HhTours_df, file = "data-raw/HhTours_df.Rda")

# And trips to 
"HhTrips_df"

### TEMP: save as RDA File
if ( exists("HhTrips_df") ) save(HhTrips_df, file = "data-raw/HhTrips_df.Rda")
...
### TEMP: save as RDA File
if ( exists("Per_df") ) save(Per_df, file = "data-raw/Per_df.Rda")


rm( list=grep("_$",ls(),value=TRUE) )
rm( list=c("Nhts2017Dir", "ToursByHh_df") )
