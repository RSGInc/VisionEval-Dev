...
if(NHTSYEAR == 2001) {
  Keep_cols2001 <- split(nhts_xwalk[,c('NHTS2001')], nhts_xwalk$File)
	...
	if (!file.exists(file.path(RAW_DIR, "Veh_df.rda"))) {
	  ...
	  Veh_df <- Veh_df[, Keep_cols[['2001']][['veh']]]
	...
	if (!file.exists(file.path(RAW_DIR, "Per_df.rda"))) {
	  ...
	  Per_df <- Per_df[, Keep_cols[['2001']][['person']]]
	...
	if (!file.exists(file.path(RAW_DIR, "Dt_df.rda"))) {
	  ...
	  Dt_df <- Dt_df[, Keep_cols[['2001']][['day/trip']]]
...
if(NHTSYEAR == 2017) {
	...
	if (!file.exists(file.path(RAW_DIR, "Veh_df.rda"))) {
	  ...
	  Veh_df <- Veh_df[, Keep_cols[['2017']][['veh']]]
	...
	if (!file.exists(file.path(RAW_DIR, "Dt_df.rda"))) {
	  ...
	  Dt_df <- Dt_df[, Keep_cols[['2017']][['day/trip']]]
	...
	if (!file.exists(file.path(RAW_DIR, "Per_df.rda"))) {
	  ...
	  Per_df <- Per_df[, Keep_cols[['2017']][['person']]]
...
rm(list=ls()[!(ls() %in% c('Hh_df', 'Per_df', 'Veh_df', 'HhTours_df'))])
