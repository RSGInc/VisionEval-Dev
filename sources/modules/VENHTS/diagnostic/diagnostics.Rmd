...
compare_plot <- function(var) {
  ...
  if(compare_vars[var] == 'frequency') {
    ...
    plot <- ggplot(agg, aes(x=UQ(as.name(var)), y=pct, fill=NHTS)) + 
      ...
      scale_y_continuous("Percent of sample", labels=scales::percent_format(accuracy=1))
  } else {
...
```{r hh_plots, eval=T, out.width="100%", warning=TRUE}
chunker <- function(x, n) split(x, sort(x %% (length(x) %/% n)))

hh_plots <- lapply(names(compare_vars), compare_plot)
chunks <- chunker(1:length(hh_plots), 4)
